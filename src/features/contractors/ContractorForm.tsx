...
function getInitialFormData(contractor?: ContractorFormData | null): ContractorForm {
    return {
        name: contractor?.name ?? '',
        inn: contractor?.inn ?? '',
        kpp: contractor?.kpp ?? null,
        ogrn: contractor?.ogrn ?? null,
        address: contractor?.address ?? null,
        phone: contractor?.phone ?? null,
        email: contractor?.email ?? null,
        contact_person: contractor?.contact_person ?? null,
    };
}
...
export default function ContractorForm({
    ...
    const [formData, setFormData] = useState<ContractorForm>(() => getInitialFormData(contractor));
    ...
    const validate = () => {
        ...
        } else if (formData.inn.length !== 14) {
        ...
        } else if (formData.kpp.length !== 9) {
            newErrors.kpp = 'КПП должен содержать 9 цифр';
        } else if (!/^\d+$/.test(formData.kpp)) {
            newErrors.kpp = 'КПП должен содержать только цифры';
        }
        ...
        } else if (!/^\d+$/.test(formData.ogrn)) {
            newErrors.ogrn = 'ОГРН должен содержать только цифры';
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload: ContractorFormData = {
            ...formData,
            inn: formData.inn!,
            kpp: formData.kpp!,
            ogrn: formData.ogrn!,
    ...
    /*Изменение полей*/
    const handleChange = <K extends keyof ContractorForm>(field: K, value: ContractorForm[K]) => {
        setFormData((prev) => ({
            ...prev,
            [field]: value,
        }));
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2">
                        ...
                        <textarea
                            ...
                            `}
                            placeholder="Введите адрес подрядчика"
                            disabled={loading}
                        />
                        {errors.address && (
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-gray-200">
                ...
                >
                    ...
                    {contractor ? 'Сохранить изменения' : 'Создать подрядчика'}
                </button>
                <button
                    type="button"
                    onClick={onCancel}
