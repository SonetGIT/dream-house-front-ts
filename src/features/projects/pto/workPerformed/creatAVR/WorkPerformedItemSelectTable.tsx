...
export default function WorkPerformedItemSelectTable({
    items,
    loading,
    refs,
    calcRowTotal,
    ...
    const serviceItems = items.filter((i) => i.item_type === 2); //услуги

    //SELECT
    const isAllSelected = useMemo(() => {
        return serviceItems.length > 0 && selectedIds.length === serviceItems.length;
    }, [selectedIds, serviceItems]);

    const toggleAll = () => {
        if (isAllSelected) {
            setSelectedIds([]);
        } else {
            setSelectedIds(serviceItems.map((i) => i.id));
        }
    };
    ...
    const selectedItems = useMemo(() => {
        return serviceItems.filter((i) => selectedIds.includes(i.id));
    }, [selectedIds, serviceItems]);
                    ...
                    {serviceItems.map((sub) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <td className="px-3 py-3 text-sm">
                                    {refs.serviceTypes.lookup(Number(sub.service_type))}
                                </td>

                                <td className="px-3 py-3 text-sm">
                                    {refs.services.lookup(Number(sub.service_id))}
                                </td>
